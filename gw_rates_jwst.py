...
#     parser.add_argument('--ntry', default=1, type=int, action=MinZeroAction, help='Set the number of MC samples')
...
#             lc = SEDDerviedLC(mej = mej, phi = phi, cos_theta = cos_theta, dist=d)
#             bools = lc.detectionBoolLSST()
#             can_be_detected = np.any(list(bools.values()))
#             em_bool.append(can_be_detected)

#         em_bool = np.array(em_bool)


        
#         # TODO: Get the LC's and covert to apparent mag based on dist


#         # TODO: Find phase when mag crosses the threshold
...
#     fig.savefig(f'gw_detect_{args.mass_distrib}_JWST.pdf')
#     plt.show()
