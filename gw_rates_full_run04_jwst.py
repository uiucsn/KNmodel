...
def main(argv=None):
    ...
    # H-band magnitude as function of phase (observer frame) at 40 Mpc
    temphmag  = temp['f160w']
    # R-band magnitude as function of phase (observer frame) at 40 Mpc
    temprmag  = temp['f625w']
    ...
    def dotry(n):
        ...
        # this is some sort of delay that only applies to the first three months
        # unclear why we had this. Disabling 20230127
        delay = np.random.uniform(0, 365.25, n_events)
        #delay[delay > 90] = 0


        av = np.random.exponential(1, n_events)*0.4
        # we need the extinction in r and h bands
        ar = av*0.748
        ah = av/6.1

        sss17a_h = -16.9 #H-band
        sss17a_r = -15.8 #Rband
        minmag = -14.7
        maxmag = sss17a_r - 2.
