...
if __name__=='__main__':
    ...
    axes[0][0].axvline(mean_nevents, color='C0', linestyle='--', lw=2,
                    label=r'$\langle N\rangle = %.2f ;~ N_{95} = %.2f$' % (mean_nevents, ninetyfive_percent))
    axes[0][0].axvline(ninetyfive_percent, color='C0',
    ...
    #print("Test")
    #print(test)
    axes[0][0].hist(n_detect3, density=True, histtype='stepfilled', color='C1', alpha=0.5, bins=ebins, zorder=1)
    axes[0][0].hist(n_detect3, density=True, histtype='step', color='C1', lw=3, bins=ebins, zorder=2)
    ...
    axes[0][0].axvline(mean_nevents, color='C1', linestyle='--', lw=2,
                label=r'$\langle N\rangle = %.2f ;~ N_{95} = %.2f$' % (mean_nevents, ninetyfive_percent))
    axes[0][0].axvline(ninetyfive_percent, color='C1',
                linestyle='dotted', lw=1)
    #vals, bins = np.histogram(n_detect4, bins=ebins, density=True)
    mean_nevents = np.mean(n_detect4)
    # #vals*=nor
    # #test = dict(zip(ebins, vals))
    axes[0][0].hist(n_detect4, density=True, histtype='stepfilled', color='C2', alpha=0.5, bins=ebins, zorder=1)
    axes[0][0].hist(n_detect4, density=True, histtype='step', color='C2', lw=3, bins=ebins, zorder=2)
    five_percent, ninetyfive_percent = np.percentile(n_detect4, 5), np.percentile(n_detect4, 95)
    axes[0][0].axvline(round(mean_nevents), color='C2', linestyle='--', lw=2,
                    label=r'$\langle N\rangle = %.2f ;~ N_{95} = %.2f$' % (mean_nevents, ninetyfive_percent))
    axes[0][0].axvline(ninetyfive_percent, color='C2',
                    linestyle='dotted', lw=1)
    axes[0][0].legend(frameon=False, fontsize='medium', loc='upper right')
    #axes[0].set_xscale('log')
    axes[0][0].set_yscale('log')
    axes[0][0].set_xlim((0., 31))
    ...
    try:
        ...
        axes[0][1].plot(dist_range, pdist, color='C0', linestyle='-', lw=3, zorder=4)
        patch1 = axes[0][1].fill_between(dist_range, np.zeros(len(dist_range)), pdist, color='C0', alpha=0.3, zorder=0)
        patches.append(patch1)
        legend_text.append('2 Detector Events')
        mean_dist = np.mean(dist_detect2)
        axes[0][1].axvline(mean_dist, color='C0', linestyle='--', lw=1.5, zorder=6, label=r'$\langle D \rangle = {:.0f}$ Mpc'.format(mean_dist))
    ...
    try:
        ...
        pdist = kde(dist_range)
        axes[0][1].plot(dist_range, pdist, color='C1', linestyle='-', lw=3, zorder=2)
        patch2 = axes[0][1].fill_between(dist_range, np.zeros(len(dist_range)), pdist, color='C1', alpha=0.5, zorder=1)
    ...
    try:
        ...
        axes[0][1].plot(dist_range, pdist, color='C2', linestyle='-', lw=3, zorder=2)
        axes[0][1].axvline(mean_dist, color='C2', linestyle='--', lw=1.5, zorder=6, label=r'$\langle D \rangle = {:.0f}$ Mpc'.format(mean_dist))
        patch3 = axes[0][1].fill_between(dist_range, np.zeros(len(dist_range)), pdist, color='C2', alpha=0.5, zorder=1)
    ...
    axes[1][0].plot(h_range, ph, color='C0', linestyle='dotted', lw=3, zorder=4, label='Detection mag')
    axes[1][0].plot(h_range, peak_ph, color='C0', linestyle='-', lw=3, zorder=4, label='Peak mag')
    axes[1][0].fill_between(h_range, np.zeros(len(h_range)), peak_ph, color='C0', alpha=0.3, zorder=0)
    mean_h = np.mean(mag_peak2)
    axes[1][0].axvline(mean_h, color='C0', linestyle='--', lw=1.5, zorder=6, label=r'$\langle r \rangle = {:.1f}$ mag'.format(mean_h))
    ...
    axes[1][0].plot(h_range, ph, color='C1', linestyle='dotted', lw=3, zorder=2, label='Discovery mag')
    axes[1][0].plot(h_range, peak_ph, color='C1', linestyle='-', lw=3, zorder=2, label='Peak mag')
    axes[1][0].fill_between(h_range, np.zeros(len(h_range)), peak_ph, color='C1', alpha=0.5, zorder=1)
    mean_h = np.mean(mag_peak3)
    axes[1][0].axvline(mean_h, color='C1', linestyle='--', lw=1.5, zorder=6, label=r'$\langle r \rangle = {:.1f}$ mag'.format(mean_h))
    axes[1][0].legend(frameon=False, fontsize='medium')

    try:
        kde = spstat.gaussian_kde(mag_detect4, bw_method='scott')
        ph = kde(h_range)
        axes[1][0].plot(h_range, ph, color='C2', linestyle='-', lw=3, zorder=2)
        axes[1][0].fill_between(h_range, np.zeros(len(h_range)), ph, color='C1', alpha=0.5, zorder=1)
    ...
    axes[0][1].set_ylabel('$P(D)$', fontsize='x-large')
    ...
    axes[0][0].set_xlabel('Number of Events ($N$)', fontsize='x-large')
    axes[0][0].set_ylabel('$P(N)$', fontsize='x-large')

    axes[1][0].set_xlabel('Apparent ($r$, AB mag)', fontsize='x-large')
